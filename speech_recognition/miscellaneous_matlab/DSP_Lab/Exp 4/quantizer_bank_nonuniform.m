function Q = quantizer_bank_nonuniform(Y,number_of_levels_vector)
...
for n = 1:size_size(1)
    ...
    for iterator = 1:length(Y(n,:))
        ...
        lowest_level = (2/number_of_levels_vector(n)) - 1;
        if (Y(n,iterator) < lowest_level)
            ...
            % levels
            for i = 1:number_of_levels_vector(n)
                level = (i*2/number_of_levels_vector(n)) - 1;
         
                if ((Y(n,iterator) > level) && (Y(n,iterator) < level+2/number_of_levels_vector(n)))
                    Q(n,iterator) = level;
                elseif((Y(n,iterator) > level))
                    Q(n,iterator) = level+2/number_of_levels_vector(n);
                end
            end
        end
    end
