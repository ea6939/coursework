...
void seven_segment_GPIO_setup(void)
{
	...
	gpio_init_s.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_2 | GPIO_Pin_4 | GPIO_Pin_5 | GPIO_Pin_7 | GPIO_Pin_8 | GPIO_Pin_11 | GPIO_Pin_12 | GPIO_Pin_13 | GPIO_Pin_14;
...
void display_multidigit_number(float temp) 
{
	...
	delay(200000); // need delay to give the LEDs more time to light up during switching
...
/*!
@brief no-op
@param n the duration (no units, depends on compiler) to delay
Software delay (avoid!!)
Decrements n until n = 0; causes delay.
*/
void delay(int n)
{
	while(n--);
